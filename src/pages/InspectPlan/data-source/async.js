...
export const getAllProjectListRequest = async () => {
    return await requestWrapper(() => service.get(API.GET_PROJECT_LIST_FOR_INSPECT), '获取项目信息失败');
}
...
export const publishPlanRequest = async params => {
    const data = await requestWrapperWithoutData(() => xApi.post(`${API.PUBLISH_CHECK_PLAN}`, params),'创建检查计划失败');
    return data?.success;
}
